...
class App {
    apiService;
    renderService;
    constructor(apiService, renderService) {
    ...
    async start() {
        try {
            ...
            this.renderService.showError(error instanceof Error ? error.message : 'Unknown error');
        }
    }
}
class ApiService {
...
async function getUserPost() {
    try {
        ...
        if (!responsePost.ok)
            throw new Error("Server Error");
        const userPosts = await responsePost.json();
        const titles = userPosts.map(post => post.title);
