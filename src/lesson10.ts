...
const historyPriceValueDocuments: number[] = Array.from({ length: 5 }, () => Math.floor(Math.random() * 2000));
...
// ====== 2 ======
const indexHigherMoreThan1000: number[] = historyPriceValueDocuments.reduce((acc, num, index) => {
  if (num > 1000) {
    acc.push(index)
  }
  return acc
...
// ====== 6 ======
const isPriceLess1000: boolean = historyPriceValueDocuments.some(price => price < 1000);
document.write(`The price less than 1000 : ${isPriceLess1000} <br>`)
console.log(isPriceLess1000);

// ====== 7 ======
const isAllPricesMore1000: boolean = historyPriceValueDocuments.every(price => price > 1000);
...
historyPriceValueDocuments.forEach(price => {
  if (price > 1000) return countSumPrices += price
})
